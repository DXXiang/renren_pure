...
import io.sdses.modules.sys.dao.SysUserDao;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserDao, SysUserEntity> implements SysUserService {
	...
	@Override
	public boolean updateStatus(List<Long> userIdList) {
        SysUserEntity userEntity = new SysUserEntity();
        userEntity.setStatus(2);
        boolean flag = false;       
    	for(Long userId : userIdList){
    		flag = this.update(userEntity,
	...
	public boolean isContainUserNameForSave(String username){
		List<Long> userList = baseMapper.isContainUserNameForSave(username);
		if(userList.size()<=0){
			return false;
		}
		return true;
	...
	public boolean isContainUserNameForUpdate(String username, Long userId){
		List<Long> userList = baseMapper.isContainUserNameForUpdate(username, userId);
		if(userList.size()<=0){
			return false;
		}
		return true;
	...
	public boolean deleteUserRole(Long[] userIds){
	    ...
	    return baseMapper.deleteUserRole(userIds);
